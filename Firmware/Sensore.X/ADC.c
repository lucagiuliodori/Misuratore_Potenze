        ...
        switch(adc_ch)
        {
            ...
            case ADC_CH_CURR:
                ...
                if(cntSum==MAX_CNTSUM_CURR)
                {
                    ...
                    rmsCurr=(float)sumCurr;
                    rmsCurr*=(float)1.024;
                    rmsCurr*=(float)1.024;
                    rmsCurr*=(float)WIND_RATIO;
                    rmsCurr*=(float)WIND_RATIO;
                    rmsCurr/=(float)1024;
                    rmsCurr/=(float)1024;
                    rmsCurr/=(float)RES;
                    rmsCurr/=(float)RES;
                    rmsCurr/=(float)MAX_CNTSUM_CURR;
                ...
                {
                    //Legge e somma il valore (approssimato a 8 bit).
                    sumCurr+=((ADRESH<<8|ADRESL)*(ADRESH<<8|ADRESL));     //Non viene usata la funzione "pow" per ottimizzare lo spazio usato e per velocizzare le operazioni.
                    //Incrementa il contatore.
                    cntSum++;
                }
                break;
