...
typedef struct _PGM_CFG
{
    ...
    unsigned char gen_curr_sens_id;
    unsigned char con_curr_sens_id;
    unsigned char volt_sens_id;
}ts_PGM_CFG;

typedef struct _PGM_PGM_CFG
{
    ts_PGM_CFG pgm_cfg;
    unsigned char dummy[PGM_PAGESIZE-sizeof(ts_PGM_CFG)];   //Questo buffer "dummy" è stato inserito per riservare una pagina di PGM per le config (utile durante la modifica delle config).
}ts_PGM_PGM_CFG;
